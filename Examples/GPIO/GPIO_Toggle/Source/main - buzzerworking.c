   ...
   //      GPIO_ClearBit(OUTPUT_PORT, PUMP_GPIO_PIN);
    //   } 
		 ...
		 if(SYSTEM_ON)
		 {
				 ...
				 // E0 buzzer, LED, and alarm control - runs every cycle regardless of numbers_visibilty_timer
				 if(Error==0)
				 {
					 if(error_visibility_timer==1)
					 {
						 // Turn ON: alarm symbols, LED, and buzzer
						 Tap=true;
						 Bell=true;
						 BellVib=true;
						 GPIO_SetBit(BuzzerLed_GPIO_PORT,Led_GPIO_PIN);
						 if(buzzer_error_timer!=0 || onetime_buzzer)
						 {
							 GPIO_SetBit(BuzzerLed_GPIO_PORT,Buzzer_GPIO_PIN);
						 }
					 }
					 else
					 {
						 // Turn OFF: alarm symbols, LED, and buzzer
						 Tap=false;
						 Bell=false;
						 BellVib=false;
